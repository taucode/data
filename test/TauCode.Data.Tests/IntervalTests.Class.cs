...
[TestFixture]
public class IntervalTestsClass
{
    [Test]
    ...
    public void Ctor_AnyArgument_ReturnsExpectedResult(CtorTestDto testDto)
    {
        ...
        Interval<IntClass> interval;

        // Act & Assert
        if (testDto.ExceptionException == null)
        {
            interval = new Interval<IntClass>(
                intervalDto.Start!,
                intervalDto.End!,
                intervalDto.IsStartIncluded,
                intervalDto.IsEndIncluded);

            Assert.That(interval.Start, Is.EqualTo((IntClass)testDto.ExpectedInterval!.Start!));
            Assert.That(interval.End, Is.EqualTo((IntClass)testDto.ExpectedInterval.End!));
    ...
    {
        // Arrange
        var interval = new Interval<IntClass>(10!, 20!, true, true);
    ...
    public void CreateEmpty_NoArguments_CreatesEmptyInterval()
    {
        ...
        // Act
        var interval = Interval<IntClass>.CreateEmpty();

        // Assert
        Assert.That(interval.Start, Is.EqualTo((IntClass)0!));
        Assert.That(interval.End, Is.EqualTo((IntClass)0!));
    ...
    public void CreateSingleValue_ValidValue_CreatesSingleValueInterval()
    {
        ...
        // Act
        var interval = Interval<IntClass>.CreateSingleValue(17!);

        // Assert
        Assert.That(interval.Start, Is.EqualTo((IntClass)17!));
        Assert.That(interval.End, Is.EqualTo((IntClass)17!));
    ...
    public void CreateSingleValue_ArgumentIsNull_ThrowsArgumentNullException()
    {
        ...
        // Act
        var ex = Assert.Throws<ArgumentNullException>(() => Interval<IntClass>.CreateSingleValue(null!));
    ...
    private static Interval<IntClass> DtoToInterval(IntervalDto dto)
    {
        return new Interval<IntClass>(
            dto.Start!,
            dto.End!,
            dto.IsStartIncluded,
            dto.IsEndIncluded);
